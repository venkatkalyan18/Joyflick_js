...
export default function FlipFind() {
  ...
  return (
    <main className="w-full bg-background1 flex items-center p-8 flex-col font-thin">
      <div className="flex flex-col w-full items-center gap-2">
        <h1 className="text-5xl font-bold text-white font-mono max-sm:text-3xl">
          Flip & Find
        </h1>
        <div>
          ...
          <input
            ...
            min="4"
            max="6"
            className="border border-gray-400 rounded text-center w-16 p-1"
            onChange={(e) => handleSizeChange(e)}
          />
        </div>
        {isSizeExceed && (
          <h1 className="text-red-500">Grid size should be between 4 - 6</h1>
        )}
      </div>
      <section className="flex w-fit p-10 mt-4 max-sm:mt-4  justify-start items-center flex-col gap-3 ">
        <div
          className={`grid grid-cols-${gridSize} gap-3  bg-[#2f2f4f] p-8 max-sm:p-5 `}
        >
          {isGameOver ? (
            <div className="w-full h-full text-3xl text-white font-bold rounded-lg">
              Game Won!{" "}
            </div>
          ) : (
              ...
              >
                {row.map((col, colIndex) => (
                  <div className="relative">
                    <div
                      className={`bg-[#ba347b] w-16 h-16 absolute ${
                        !gridMap[rowIndex][colIndex] ? "block" : "hidden"
                      } ${
                        gridSize == 6
                          ? "max-md:w-10 max-md:h-12"
                          : "max-md:w-12 max-md:h-14"
                      } cursor-pointer`}
                      onClick={() =>
                        !isProcessing && handleClick(rowIndex, colIndex)
                      }
                    ></div>

                    <p
                      className={`w-16 h-16 flex justify-center items-center bg-slate-300 ${
                        gridSize == 6
                          ? "max-md:w-10 max-md:h-12"
                          : "max-md:w-12 max-md:h-14"
                      } font-bold`}
        ...
        <button
          className=" bg-black text-white rounded-lg px-6 py-3 text-lg font-bold w-full"
