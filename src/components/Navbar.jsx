...
import {
  ...
  SheetTitle,
  SheetTrigger,
} from "./ui/sheet"; // Ensure the correct import path
import Menu from "../assets/menu.svg";

const Navbar = () => {
  const location = useLocation();

  const getTextForRoute = (path) => {
    switch (path) {
      case "/drench":
        return "- Drench";
      ...
      case "/flip-find":
        return "- Flip and Find";
      case "/sand-box":
        return "- Sand Simulator";
      default:
        return "";
    }
  };
  return (
    <nav className="flex items-center justify-between py-4 px-12 max-md:px-8 bg-black text-white font-mono">
      <div className="flex flex-row gap-5">
        ...
        <h1 className="text-2xl">{getTextForRoute(location.pathname)}</h1>
      </div>
      ...
      <div className="md:hidden">
        <Sheet>
          ...
          <SheetContent className="flex flex-col gap-10 bg-white md:hidden font-mono w-[300px]">
            {" "}
            {/* Reduced Width */}
            <SheetHeader>
              <SheetTitle className="text-2xl font-mono mb-4">
                Joy Flick
              </SheetTitle>
